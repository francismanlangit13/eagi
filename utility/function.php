...
function get_total_products_seller($con){
	$seller_id=$_SESSION['SELLER_ID'];
	$n=mysqli_num_rows(mysqli_query($con,"select * from product where added_by='$seller_id'"));
	return $n;
}
...
function get_total_user($con){
	$n=mysqli_num_rows(mysqli_query($con,"select * from users"));
	return $n;
}
...
function getMyOrders($con,$type,$f=''){
	...
	foreach($allOrders as $order){
		$od=$order;
		$res=mysqli_query($con,"select orders.id,orders.o_id,order_time.added_on, orders.u_cnfrm from orders,order_time where orders.o_id='$od' and order_time.oid = orders.id and order_time.o_status = orders.order_status");
		while($row=mysqli_fetch_assoc($res)){	
			$product_arr[]=$row;
		}
	}
...
function get_total_seller_approved($con){
	$n=mysqli_num_rows(mysqli_query($con,"select * from sellers where isapp='1'"));
	return $n;
}
function get_total_seller_pending($con){
	$n=mysqli_num_rows(mysqli_query($con,"select * from sellers where isapp='0'"));
	return $n;
}
function get_total_seller_rejected($con){
	$n=mysqli_num_rows(mysqli_query($con,"select * from sellers where isapp='2'"));
	return $n;
}
function plan($con){
	$seller_id=$_SESSION['SELLER_ID'];
...
function privilage($con){
	$admin_id=$_SESSION['ADMIN_ID'];
	$row=mysqli_fetch_assoc(mysqli_query($con,"select * from admin where id='$admin_id'"));
	return $row['privilage'];
}
...
function update_cart($con,$cart_id){
	...
	$product_id=$product['added_by'];
	$f=mysqli_fetch_assoc(mysqli_query($con,"select * from cart where id='$cart_id'"));
	$fn=mysqli_fetch_assoc(mysqli_query($con,"select * from dc where id='$product_id'"));
	if($totalPrice<=$fn['dc']){
		$ship= $fn['pc'];
	}else{
		$ship= 0;
